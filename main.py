...
def ejecutar_proyecto():
    ...
    fecha_especifica = pd.Timestamp('21-11-2021 00:00:00')
    ...
    for i in range(1, 3):
        ...
        if not datos_repetidos.empty:
            ax1.scatter(datos_repetidos['id'], datos_repetidos['historia'],
                        label=f'Datos Repetidos ({bases_de_datos[0].nombre} vs {bases_de_datos[i].nombre})')
            ax1.set_xlabel('ID')
